...
% =================================================================================================================================
% ALGORITHM FOR EVOLVING A SOCIO-ECONOMIC SPACE WITH COURNOT-NASH EXCHANGE AND ALLOWING FOR AGENTS TO MAKE MULTIPLE RELATIONSHIPS
% =================================================================================================================================
...
% New agents are entering the socio-economic space
...
for i = 2:N 
	...
	% Set the connection history to 0
	...
	X = zeros(size(R,2),N);
		...
		for s = 1:size(R,2)-1
			...
			for j = 1:i-1
				...
				Dxi = 0.5*((I(s,1) + I(s,2)) + p - 1);
				Dxj = 0.5*((C(j,1) + (C(j,2) * p)) + p - 1);
				
				r = solve(Dxi + Dxj == C(j,1) + I(s,1), p);
				
				Ir = [m, 0; 0, (k * r); I(3,1), (I(3,2) * r)];

				T(1,j) = 0.5*((Ir(s,1) + Ir(s,2)) + r - 1);
				T(2,j) = 0.5*((Csx(j,s) + (Csy(j,s) * r)) + r - 1);
				T(3,j) = ((Ir(s,1) + (Ir(s,2))) - r + 1)/(2 * r);
				T(4,j) = ((Csx(j,s) + (Csy(j,s) * r)) - r + 1)/(2 * r);
				T(5,j) = (T(1,j) + 1) * (T(3,j) + 1);

				% i can only connect to j if it is benefical for j (consent)
				% If not then j will not accept to i

				if (T(2,j) + 1) * (T(4,j) + 1) < Us(j,s);
					T(5,j) = 0;
					T(6,j) = 0;
			...
			for x = 1:i-1
				...
				for j = 1:i-1
					...
					Dxi = 0.5*((Csx(i,s) + (Csy(i,s) * p)) + p - 1);
					Dxj = 0.5*((Csx(j,s) + (Csy(j,s) * p)) + p - 1);
					
					r = solve(Dxi + Dxj == Csx(j,s) + Csx(i,s), p);

					L(1,j) = 0.5*((Csx(i,s) + (Csy(i,s) * r)) + r - 1);
					L(2,j) = 0.5*((Csx(j,s) + (Csy(j,s) * r)) + r - 1);
					L(3,j) = ((Csx(i,s) + (Csy(i,s) * r)) - r + 1)/(2 * r);
					L(4,j) = ((Csx(j,s) + (Csy(j,s) * r)) - r + 1)/(2 * r);
					L(5,j) = (L(1,j) + 1) * (L(3,j) + 1);

					% i can only connect to j if it is benefical for j
					% If not then j will not accept to i

					if (L(2,j) + 1) * (L(4,j) + 1) < Us(j,s);
						L(5,j) = 0;
						L(6,j) = 0;
					else L(6,j) = (L(2,j) + 1) * (L(4,j) + 1);
					end
